...
def write(x, results):
    c1 = (int(x[1]), int(x[2]))  # Ensure they are Python integers
    c2 = (int(x[3]), int(x[4]))  # Ensure they are Python integers
    img = results[int(x[0])]

    print(f"Debug: c1={c1}, c2={c2}, type(c1)={type(c1)}, type(c2)={type(c2)}")
    print(f"Debug: img.shape={img.shape}, type(img)={type(img)}")
